...
class VerifyResetCodeUseCase(
    ...
    operator fun invoke(code: String): Flow<Resource<String>> = flow {
        emit(Resource.Loading())

        val result = repository.verifyResetCode(code)
        if (result.isSuccess) {
            emit(Resource.Success(result.getOrNull() ?: "OK"))
        } else {
            emit(Resource.Error(result.exceptionOrNull()?.message ?: "Ошибка подтверждения кода"))
        }
